...
/// A stateful wrapper around a non-blocking stream. This connection is not the
/// Server connection. This connection represents the client connections
...
pub struct Connection {
    ...
    // messages waiting to the send out
    send_queue: Vec<Rc<Vec<u8>>>,

    // track whether a connection needs to be (re)registered
    is_idle: bool,

    // trace whether a connection is reset
    is_reset: bool,

    // trace whether a read received `WouldBlock` and store the number of
    // byte we are supposed to read
    read_continuation: Option<u64>,

    // trace whether a write received `WouldBlock`
...
impl Connection {
    ...
    /// The Handler must continue calling util None is returned
    ///
    /// The recieve buffer is send back to `Server` so the message can be broadcast
    /// to all listening connections
        ...
        match sock_ref.take(msg_len as u64).read(&mut recv_buf) {
            Ok(n) => {
                ...
                if n < msg_len as usize {
                    return Err(Error::new(ErrorKind::InvalidData, "Did not read enougth bytes"));
                }
            ...
            Err(e) => {
                if e.kind() == ErrorKind::WouldBlock {
                    debug!("Conn: read encounterd WouldBlock");

                    // We are being forced to try again, but we already read two
                    // tytes off of the wire that determined the length. We need
                    // to store the message length so we can resume next time we
                    // get readable
    ...
    ///
    /// Send one message the send queue to the client. If the queue is empty, remove
    /// interest in write events
    ///
    /// TODO: Figure out if sending more than one message is optimal. Maybe we should
    /// be trying to flush util the kernel sends back AGAIN?
        ...
        try!(self.send_queue.pop()
            ...
            .and_then(|buf| {
                match self.write_message_length(&buf) {
                    Ok(None) => {
                        // put messag back into the queue so we can try again
                ...
                match self.sock.write(&*buf) {
                    ...
                    Err(e) => {
                        if e.kind() == ErrorKind::WouldBlock {
                            ...
                            // put message back into the queueu so we can try again
                            self.send_queue.push(buf);
    ...
    fn write_message_length(&mut self, buf: &Rc<Vec<u8>>) -> io::Result<Option<()>> {
        ...
        match self.sock.write(&send_buf) {
            Ok(n) => {
                debug!("Send message length of {} bytes", n);
                Ok(Some(()))
            },
            Err(e) => {
                if e.kind() == ErrorKind::WouldBlock {
    ...
    /// Queue an outgong message to the client
    ///
    /// This will cause the connection to register interests in write events with the poller.
    /// The connection can still safely have an interest in read events. The read and write
    /// buffers operate independently of each other.
    ...
    ///
    /// This will let out connection accept reads starting next poller tick.
    pub fn register(&mut self, poll: &mut Poll) -> io::Result<()> {
        trace!("function connection register; connection register; token = {:?}", self.token);
